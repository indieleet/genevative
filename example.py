...
def i2(freq, dur, vel, hz):
    dur1 = 1/8
    return (np.random.rand(int(dur))*2 - 1)*np.resize(np.append(np.linspace(0, 1, int(1/8*dur))**(11/3), np.zeros(int(7/8*dur))), int(dur))
...
def i4(freq, dur, vel, hz):
    ...
    arr = np.resize(np.append(np.sin(np.linspace(-1, 1, int(freq))), np.zeros(int(freq*3/8))), int(dur))*np.resize(env, int(dur))
    bend = np.resize(np.append(np.linspace(0,1,len(arr)//8)**5, np.zeros(int(len(arr)/3*8))), int(dur))
    iar = np.interp(bend, np.linspace(0,1, len(arr)), arr)
    return iar
t = genevative.Tracker()
t.add_pattern(
        [[440, 4, 1],
         [lambda x,y,w,z:np.zeros(int(y)), 1, 1],
         [i1, 1, 2, 1],
         [i1, 1, 1/2, 1, (0,3/2),(0,64/63),(0,63/64)],
         [i1, 1, 1/3, 1],
